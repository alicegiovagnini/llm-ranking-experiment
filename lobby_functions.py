...
def send_previous_list_to_backend(previous_list_text=None, group_id=None):
    ...
    try:
        ...
        if response.status_code == 200:
            print(f"Lista precedente inviata per il gruppo {group_id}")
            return True
        else:
            print(
...
def send_message():
    ...
    if 'group_id' in st.session_state:
        group_id = st.session_state.group_id
        print(f"Invio messaggio al gruppo: {group_id}")
    ...
    if user_input and user_input.strip():
        ...
        try:
            print(f"Invio messaggio da {username} al gruppo {group_id}: {user_input}")
            ...
            print(f"Risposta dal server: {response.status_code}")
            if response.status_code != 200:
                st.error(f"Errore nell'invio del messaggio: {response.status_code}")
                print(f"Errore nell'invio del messaggio: {response.text}")
            else:
                print(f"Messaggio inviato con successo al gruppo {group_id}: {user_input}")
                fetch_messages()
        except Exception as e:
...
def register_user_rest(username):
    """
        Registra un utente attraverso l'API REST con gestione migliorata degli errori
        """
    ...
    try:
        ...
        print(f"Errore di connessione durante la registrazione: {e}")
        return False
    except Exception as e:
...
def chatroom():
    ...
    if 'group_id' in st.session_state:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                print(f"DEBUG: Membri del gruppo recuperati direttamente: {group_members}")
            else:
    ...
    # Container per i messaggi con stile
        ...
        if not st.session_state.messages:
            ...
            for msg in st.session_state.messages:
                ...
                if len(parts) == 2:
                    ...
                    if sender == username:
                        st.sidebar.markdown(f"**Tu:** {content}")
                    elif sender == "LLM":
                        st.sidebar.markdown(f"**🤖 {sender}:** {content}")
                    else:
                        st.sidebar.markdown(f"**{sender}:** {content}")
                else:
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if len(all_messages) != old_message_count:
                ...
                print(f"Recuperati {len(all_messages)} messaggi dal server per gruppo {group_id}")

                # Se ci sono nuovi messaggi, e non siamo all'inizio della conversazione,
                # forza l'aggiornamento dell'interfaccia
                if old_message_count > 0 and len(all_messages) > old_message_count:
                    print(f"Nuovi messaggi rilevati: {len(all_messages) - old_message_count}")
        else:
        ...
        if 'group_id' in st.session_state:
            try:
                # Verifica ogni 5 secondi (non ad ogni ciclo)
                if time.time() % 5 < 1:
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if group_id:
                ...
                print(f"Utente {username} appartiene al gruppo {group_id} con membri: {members}")
    ...
    while retry_count < max_retries:
        try:
            ...
            if response.status_code == 200:
                ...
                print(f"Utenti connessi recuperati: {users}")
                return users
            else:
                print(f"Errore nel recupero degli utenti: {response.status_code}")
                retry_count += 1
                time.sleep(1)
        except Exception as e:
...
def check_chat_partner(username):
    """
    Controlla se l'utente ha un partner di chat
    """
    if not username:
        return None

    # Prima controlla se già sappiamo il partner dalla sessione
    # Se esiste già un partner, ignora la verifica
    if 'partner' in st.session_state and st.session_state.partner:
        return None

    try:
        response = requests.get(f'{BASE_URL}/get_chat_partner/{username}', timeout=3)
        if response.status_code == 200:
            data = response.json()
            partner = data.get('chat_partner')
            print(f"Partner di chat per {username}: {partner}")
            return partner
        else:
            print(f"Errore nel controllo del partner. Status code: {response.status_code}")
            return None
    except Exception as e:
        print(f"Errore durante il controllo del partner: {e}")
        return None
...
def get_intervention_type(username: str, partner_or_group_id: str):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"Tipo di intervento: {intervention_type}")
    ...
    return "Ogni 3 messaggi"  # Default per la logica semplificata
...
def decline_group_invitation(request_id, from_user, to_user):
    """
    Rifiuta un invito a unirsi a un gruppo
    """
    try:
        response = requests.post(
            f"http://127.0.0.1:8000/response_request",
            json={"request_id": request_id, "from_user": from_user,
                  "to_user": to_user, "response": "decline"},
            timeout=5
        )
        if response.status_code == 200:
            return True
        else:
            print(f"Errore nel rifiuto dell'invito: {response.status_code}")
            return False
    except Exception as e:
        print(f"Errore di connessione: {e}")
        return False
...
def send_multiple_group_invitations(from_user, to_users):
    ...
    try:
        ...
        print(f"DEBUG: Creazione gruppo con ID: {group_id}, membri da invitare: {to_users}")
...
def setup_websocket_connection(username):
    ...
    if "ws_connected" in st.session_state and st.session_state.ws_connected:
        print(f"Stato WebSocket già attivo per {username}")
        return  # Evita di creare più connessioni

    async def handle_websocket_messages():
        ws_url = f"ws://127.0.0.1:8000/ws/{username}"
        print(f"Tentativo di connessione WebSocket a {ws_url}")

        while True:
            try:
                async with websockets.connect(ws_url) as websocket:
                    print(f"WebSocket connesso con successo per {username}")
                    ...
                    while True:
                        try:
                            ...
                            print(f"WebSocket: messaggio ricevuto:  {message[:100]}...")  # Primi 100 caratteri

                            # Analizza il messaggio JSON
                            data = json.loads(message)
                            msg_type = data.get("type")
                            print(f"[DEBUG WS] Tipo messaggio: {msg_type}")
                            ...
                            if msg_type == "list_sync":
                                ...
                                if sender != username and new_list:  # Solo se non è il nostro aggiornamento
                            ...
                            elif data.get("type") == "group_request_declined":
                                # Un membro ha rifiutato l'invito
                                st.session_state.notifications = st.session_state.get("notifications", [])
                                st.session_state.notifications.append({
                                    "type": "warning",
                                    "message": f"{data.get('declinedBy')} ha rifiutato l'invito al gruppo."
                                })

                                # Rimuove dalla lista dei pending invites
                                if 'pending_invites' in st.session_state:
                                    if data.get('declinedBy') in st.session_state.pending_invites:
                                        st.session_state.pending_invites.remove(data.get('declinedBy'))
                            ...
                            print("[DEBUG WS] ⚠️ Errore decodifica JSON dal messaggio WebSocket")
                            continue
                        except Exception as e:
